...
public final class CountryPostgreSqlDAO extends SqlConnectionHelper implements CountryDAO {
    ...
    public List<CountryEntity> findAll() {
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(sql)) {
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                ...
                var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_ALL.getContent() + ": Error ejecutando consulta";
                throw NoseException.create(exception, userMessage, technicalMessage);
            }
        } catch (final SQLException exception) {
            var userMessage = MessagesEnum.USER_ERROR_COUNTRY_FIND_ALL.getContent();
            var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_ALL.getContent() + ": Error preparando consulta";
            throw NoseException.create(exception, userMessage, technicalMessage);
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(sql.toString())) {
            try {
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    ...
                    var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_BY_FILTER.getContent() + ": Error ejecutando consulta";
                    throw NoseException.create(exception, userMessage, technicalMessage);
                }
            } catch (final SQLException exception) {
                var userMessage = MessagesEnum.USER_ERROR_COUNTRY_FIND_BY_FILTER.getContent();
                var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_BY_FILTER.getContent() + ": Error preparando consulta";
                throw NoseException.create(exception, userMessage, technicalMessage);
            }
        } catch (final SQLException exception) {
            var userMessage = MessagesEnum.USER_ERROR_COUNTRY_FIND_BY_FILTER.getContent();
            var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_BY_FILTER.getContent() + ": Error en conexión SQL";
            throw NoseException.create(exception, userMessage, technicalMessage);
        } catch (final Exception exception) {
        ...
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(sql)) {
            try {
                ...
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    ...
                    var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_BY_ID.getContent() + ": Error ejecutando consulta";
                    throw NoseException.create(exception, userMessage, technicalMessage);
                }
            } catch (final SQLException exception) {
                var userMessage = MessagesEnum.USER_ERROR_COUNTRY_FIND_BY_ID.getContent();
                var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_BY_ID.getContent() + ": Error preparando consulta";
                throw NoseException.create(exception, userMessage, technicalMessage);
            }
        } catch (final SQLException exception) {
            var userMessage = MessagesEnum.USER_ERROR_COUNTRY_FIND_BY_ID.getContent();
            var technicalMessage = MessagesEnum.TECHNICAL_ERROR_COUNTRY_FIND_BY_ID.getContent() + ": Error en conexión SQL";
            throw NoseException.create(exception, userMessage, technicalMessage);
        } catch (final Exception exception) {
    ...
    private CountryEntity mapResultSetToCountryEntity(ResultSet resultSet) throws SQLException {
        ...
        try {
            ...
            var technicalMessage = MessagesEnum.TECHNICAL_ERROR_INVALID_UUID.getContent() + ": ID inválido en país";
            throw NoseException.create(exception, userMessage, technicalMessage);
        }
